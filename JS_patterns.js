...
//// 1 Constructor 
class Server {
  constructor(name, ip) { this.name = name; this.ip = ip; }
...
class SimpleMembership {
  constructor(name) { this.name = name; this.cost = 50; }
}

class StandardMembership {
  constructor(name) { this.name = name; this.cost = 150; }
}
  
class PremiumMembership {
  constructor(name) { this.name = name; this.cost = 500  }
}
...
const car = {
  ...
  init() {
    console.log(`У меня есть ${this.wheels} колеса,
      мой владелец ${this.owner}`) }
}
...
class Database {
  constructor(data) { if (Database.exists) {
...
class Server {
  ...
  get url() { return `https://${this.ip}:${this.port}` }
}

function aws(server) {
  server.isAWS = true
  server.awsInfo = function() { return server.url }
  return server
}
...
//// 9 Proxy
function networkFetch(url) { return `${url} - Ответ с сервера` }
...
//////// III Behaviour Design Patterns

//// 10 Chain Of Responsibility
class MySum {
  constructor(initialValue = 42) { this.sum = initialValue }

  add(value) { this.sum += value; return this; }
}
...
//// 11 Command
class MyMath {
  constructor(initialValue = 0) { this.num = initialValue }
...
//// 12 Iterator
class MyIterator {
  constructor(data) { this.index = 0; this.data = data; }
...
class User {
  ...
  send(message, to) { this.room.send(message, this, to) }
...
//// 15 State
class Light { constructor(light) { this.light = light } }
...
//// 16 Strategy
class Vehicle { travelTime() { return this.timeTaken } }

class Bus extends Vehicle {
  constructor() { super(); this.timeTaken = 10; }
}

class Taxi extends Vehicle {
  constructor() { super(); this.timeTaken = 5; }
}

class Car extends Vehicle {
  constructor() { super(); this.timeTaken = 3; }
}

class Commute { travel(transport) { return transport.travelTime() } }
...
class Employee {
  ...
  getPaid() { return `${this.name} имеет ЗП ${this.salary}` }
}

class Developer extends Employee {
  constructor(name, salary) { super(name, salary) }
  responsibilities() { return 'процесс создания программ' }
}

class Tester extends Employee {
  constructor(name, salary) {super(name, salary) }
  responsibilities() { return 'процесс тестирования' }
}
